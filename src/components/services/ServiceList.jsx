...
const featuredServicesData = [
    {
        ...
        icon: 'cloud', iconBg: 'bg-teal-100', iconColor: 'text-teal-600',
        headerBg: 'bg-gradient-to-br from-teal-50 to-teal-100/60',
        category: 'Gas Refill', categoryBg: 'bg-teal-100 text-teal-700',
        price: '₹999', priceNote: 'starting from',
        description: 'Complete gas refilling process including leak testing and performance check for optimal cooling.',
        includes: ['Leak detection testing', 'Full gas recharge / top-up', 'Cooling efficiency check'],
...
const allServicesData = [
    {
        ...
        price: '₹999', description: 'Round-the-clock emergency AC repair service with rapid response team',
        includes: ['24/7 emergency response', 'Same day service guarantee'],
        rating: '4.8 (1,200+ bookings)',
        badge: { text: '24/7 Available', type: 'available' },
        extraTag: 'Same-day service', isEmergency: true,
    ...
    },
    {
        ...
        category: 'Cleaning', categoryBg: 'bg-blue-100 text-blue-700',
        price: '₹699', description: 'Thorough cleaning of AC unit including filters, coils, and internal components',
        includes: ['Complete filter cleaning', 'Coil inspection & wash'],
        rating: '4.8 (1,200+ bookings)',
        badge: { text: 'Popular', type: 'popular' },
    ...
    },
    {
        ...
        category: 'Maintenance', categoryBg: 'bg-purple-100 text-purple-700',
        price: '₹4,999', description: 'Complete yearly AC maintenance package with 4 scheduled services and priority support',
        includes: ['4 scheduled servicing visits', 'Priority support hotline'],
        rating: '4.8 (1,200+ bookings)',
        type: ['Split AC', 'Window AC', 'Cassette AC', 'Tower AC'],
    },
    {
        title: 'Basic AC Maintenance',
        icon: 'wrench', iconBg: 'bg-sky-100', iconColor: 'text-sky-600',
        headerBg: 'bg-gradient-to-br from-sky-50 to-cyan-50',
        category: 'Maintenance', categoryBg: 'bg-sky-100 text-sky-700',
        price: '₹499', description: 'Complete AC checkup with filter cleaning, coil inspection, and performance optimization',
        includes: ['Filter cleaning & replacement', 'Performance optimization'],
        rating: '4.8 (1,200+ bookings)',
        type: ['Split AC', 'Window AC'],
    },
    {
        title: 'Central AC Installation',
        icon: 'gearusers', iconBg: 'bg-emerald-100', iconColor: 'text-emerald-600',
        headerBg: 'bg-gradient-to-br from-emerald-50 to-teal-50',
        category: 'Installation', categoryBg: 'bg-emerald-100 text-emerald-700',
        price: '₹0', priceNote: 'Rate after site visit',
        description: 'Complete central AC system installation with ductwork, electrical setup, and commissioning',
        includes: ['Site assessment & planning', 'Full electrical & duct setup'],
        rating: '4.8 (1,200+ bookings)',
        type: ['Central AC'],
    },
    {
        title: 'Split AC Installation',
        icon: 'tools', iconBg: 'bg-indigo-100', iconColor: 'text-indigo-600',
        headerBg: 'bg-gradient-to-br from-indigo-50 to-indigo-100',
        category: 'Installation', categoryBg: 'bg-indigo-100 text-indigo-700',
        price: '₹1,500', priceNote: 'starting from',
        description: 'Professional installation of Split AC indoor and outdoor units with copper piping',
        includes: ['Wall bracket mounting', 'Copper pipe connection'],
        rating: '4.7 (950+ bookings)',
        type: ['Split AC'],
    },
    {
        title: 'Window AC Service',
        icon: 'snowflake', iconBg: 'bg-cyan-100', iconColor: 'text-cyan-600',
        headerBg: 'bg-gradient-to-br from-cyan-50 to-cyan-100',
        category: 'Service', categoryBg: 'bg-cyan-100 text-cyan-700',
        price: '₹449', priceNote: 'starting from',
        description: 'Routine maintenance and cleaning for Window AC units to ensure proper cooling',
        includes: ['Front panel cleaning', 'Filter washing'],
        rating: '4.6 (600+ bookings)',
        type: ['Window AC'],
    },
    {
        title: 'Cassette AC Repair',
        icon: 'fan', iconBg: 'bg-stone-100', iconColor: 'text-stone-600',
        headerBg: 'bg-gradient-to-br from-stone-50 to-stone-100',
        category: 'Repair', categoryBg: 'bg-stone-100 text-stone-700',
        price: '₹1,299', priceNote: 'starting from',
        description: 'Expert repair services for commercial cassette ceiling air conditioners',
        includes: ['Drain pump checking', 'Sensor calibration'],
...
const ServiceCard = ({ service }) => {
    const handleCardClick = () => {
        const url = `https://wa.me/919770816132?text=${encodeURIComponent(`Hi, I'm interested in your ${service.title} service!`)}`;
        ...
        >
            ...
            <div className={`${service.headerBg} px-5 pt-5 pb-4 border-b border-gray-100`}>
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        {service.badge && (
                            <span className={`inline-block text-[11px] font-bold px-2 py-0.5 rounded-full mb-1.5 ${service.badge.type === 'available' ? 'bg-orange-500 text-white' : 'bg-red-600 text-white'}`}>
                                {service.badge.text}
                            </span>
                        )}
            ...
            <div className="px-5 pt-4 pb-5 flex flex-col flex-grow">
                ...
                <div className="mb-4">
                    ...
                    <span className="text-ac-text-muted text-sm ml-1">{service.priceNote || ''}</span>
                </div>
                <div className="mb-4 flex-grow">
                    ...
                    <ul className="space-y-1.5">
                        {service.includes.map((item, idx) => (
                            <li key={idx} className="flex items-start gap-2 text-[13px] text-ac-gray-dark/80">
                                <SVGIcon name="checkcircle" size={16} className="text-green-500 flex-shrink-0 mt-0.5" />
                                <span>{item}</span>
                            </li>
                        ))}
                    </ul>
                    <a href="#contact" onClick={(e) => e.stopPropagation()}
                        ...
                        <SVGIcon name="eye" size={12} /> View all inclusions
                    </a>
                </div>
                <div className="flex flex-col gap-3 mt-auto">
                    <Link href="/booking" onClick={(e) => e.stopPropagation()}
                        className={`w-full py-2.5 rounded-xl font-semibold text-center text-[13.5px] transition-all duration-300 shadow flex items-center justify-center gap-1.5 ${service.isEmergency ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-[#18a749] text-white hover:bg-green-700'}`}>
                        {service.isEmergency
                            ? <><SVGIcon name="phone" size={14} /> Book Now</>
                            : <><SVGIcon name="whatsapp" size={16} /> Book Now</>
                        }
                    </Link>
                    <div className="w-full flex items-center gap-1 flex-wrap">
                        <SVGIcon name="star" size={13} className="text-yellow-400" />
            ...
            <div className="max-w-[1200px] mx-auto px-6">
                ...
                <div className="mb-14">
                    <h3 className="text-[20px] font-bold text-gray-800 mb-6 flex items-center gap-2">
                        <SVGIcon name="star" size={18} className="text-yellow-500" /> Featured Services
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {featuredServicesData.map((service, index) => (
                            <ServiceCard service={service} key={`feat-${index}`} />
                        ))}
                    </div>
                </div>
                    ...
                    {filteredServices.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredServices.map((service, index) => (
                                <ServiceCard service={service} key={`grid-${index}`} />
                            ))}
                        </div>
                    ) : (
                        <div className="text-center py-20 bg-white border border-gray-200 rounded-2xl">
                            ...
                            <button onClick={() => { setSearchQuery(''); setActiveFilter('All Services'); }}
